...
class UT_Elementor_Addons_Lite_Widget_Blog_List extends Widget_Base {
  ...
  public function get_icon() {
      return 'eicon-post-list ut-widget-icon';
  }
  ...
  protected function register_controls() {	
    ...
    $this->add_responsive_control(
        'ut_divider_gap',
        [
            'label' => esc_html__( 'Divider Gap', 'ut-elementor-addons-lite' ),
            'type' => Controls_Manager::SLIDER,
            'default' => [
                'size' => 15,
            ],
    ...
    $this->add_responsive_control(
        'ut_viewall_border_radius_hover',
        [
            'label' => esc_html__( 'Border Radius', 'ut-elementor-addons-lite' ),
            'type' => Controls_Manager::DIMENSIONS,
            'size_units' => [ 'px', '%', 'em' ],
    ...
    $this->add_group_control(
        Group_Control_Box_Shadow::get_type(),
        [
            'name' => 'ut_viewall_box_shadow_hover',
            'label' => esc_html__( 'Box Shadow', 'ut-elementor-addons-lite' ),
        ...
        <div <?php echo $this->get_render_attribute_string( 'utal-blog-list' ); ?>>
            ...
            <div class="utal-blog-list-wrapper">
                ...
                if ( $query->have_posts() ):
                    ...
                    endwhile;
                    wp_reset_postdata();         
                    if ( ! empty( $settings['ut_viewall_button'] ) ) {                        
                        $target = isset( $settings['ut_viewall_button_link']['is_external'] ) ? ' target="_blank"' : '';
                        $nofollow = isset( $settings['ut_viewall_button_link']['nofollow'] ) ? ' rel="nofollow"' : '';
                        $url = isset( $settings['ut_viewall_button_link']['url'] ) ? $settings['ut_viewall_button_link']['url'] : '';
                        echo '<div class="viewall-button"><a href="' . esc_url( $url ) . '"' . $target . $nofollow . '>' . $settings['ut_viewall_button'] . ' </a></div></div></div>';
                    }
                endif;                
                ?>
            </div>
